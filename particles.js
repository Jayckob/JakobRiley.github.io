...
var pJS = function(tag_id, params){
      ...
      if(pJS.particles.line_linked.enable || pJS.particles.move.attract.enable){
        ...
        for(var j = 0; j < pJS.particles.array.length; j++){
			...
			if(pJS.particles.line_linked.enable){
				if(pJS.fn.interact.linkParticles(p,p2)){
					numberoflinks++;
				}
			}
			if(numberoflinks > 4){
  ...
  pJS.fn.interact.attractParticles  = function(p1, p2){
    ...
    /* condensed particles */
    var dx = Math.abs(Math.max(p1.x,p2.x) - Math.min(p1.x,p2.x));
    var dy = Math.abs(Math.max(p1.y,p2.y) - Math.min(p1.y,p2.y));
    ...
    if(dist <= p1.radius * 10){
		...
		p1.vx -= (ax * multco)/(dist/1000.0);
		p1.vy -= (ay * multco)/(dist/1000.0);
		//p1.vx = p1.vx * 0.1;
		//p1.vy = p1.vy * 0.1;
    }
	...
	if(dist >= p1.radius * 10 && dist <= p1.radius * 30
		...
		p1.vx += (ax * multco * 10.0) / (dist);
		p1.vy += (ay * multco * 10.0) / (dist);
    }
...
/*
    if(dist <= 300){
		  ...
		  var ax = (pJS.particles.move.attract.rotateX/dx),
			  ay = (pJS.particles.move.attract.rotateY/dy);
		...
		if(p1.vy > topspeed + 3 || p1.vx > topspeed + 3){
			p1.vy = 0;
			p1.vx = 0;
		}
